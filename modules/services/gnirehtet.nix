...
let
  ...
  args = "-d '${concatStringsSep "," cfg.dns}' "
         + "-p '${toString cfg.port}' "
...
in {
  ...
  config = mkIf cfg.enable {
    ...
    systemd.user.services."adb-wait-device@" = {
      Unit = {
        Description = "Wait for ADB device %I to be connected";
      };

      Service = {
        Type = "oneshot";
        RemainAfterExit = true;
        ExecStart = "${pkgs.coreutils}/bin/timeout 30 ${pkgs.androidenv.androidPkgs_9_0.platform-tools}/bin/adb -s %I wait-for-device";
      };
    };
    ...
    systemd.user.services.gnirehtet-relay = {
      ...
      Service = {
        ExecStart = "${pkgs.gnirehtet}/bin/gnirehtet relay -p ${toString cfg.port}";
      };
    };
    ...
    systemd.user.services."gnirehtet-client@" = {
      ...
      Service = {
        Type = "oneshot";
        RemainAfterExit = true;
        ExecStartPre = "${pkgs.coreutils}/bin/timeout 20 ${pkgs.androidenv.androidPkgs_9_0.platform-tools}/bin/adb -s %I wait-for-device";
